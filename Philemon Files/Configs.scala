...
object GemminiConfigs {
  val defaultConfig = GemminiArrayConfig[SInt, Float, Float](
    ...
    dataflow = Dataflow.WS, // BOTH

    // Scratchpad and accumulator
    sp_capacity = CapacityInKilobytes(2048), // 256
    acc_capacity = CapacityInKilobytes(2048), // 64
    ...
    reservation_station_entries_ld = 8, // 8
    reservation_station_entries_st = 4, // 4
    reservation_station_entries_ex = 16, // 16

    // Ld/Ex/St instruction queue lengths
    ld_queue_length = 32, // 8
    st_queue_length = 32, // 2
    ex_queue_length = 32, // 8

    // DMA options
    max_in_flight_mem_reqs = 16,

    dma_maxbytes = 64,
    dma_buswidth = 512, // 128
  ...
  // --- UCLA SysArch ($54 mm^2$ Target) ---
  val uclaConfig = defaultConfig.copy(
    meshRows=64,       
    meshColumns=64,
    tileRows=1,        
    tileColumns=1,     
    // --- Parallel Fill Optimizations ---
    dma_maxbytes=64,
    dma_buswidth=512,           // Wide pipe for parallel fill
    ld_queue_length=32,         
    ex_queue_length=32,
    st_queue_length=32,         // Increase this to match load depth
    sp_banks=16,                // 16 banks for parallel input
    acc_banks=16,               // 16 banks for parallel output
...
// SysArch
class UCLAGemminiConfig[T <: Data : Arithmetic, U <: Data, V <: Data](
  gemminiConfig: GemminiArrayConfig[T,U,V] = GemminiConfigs.uclaConfig
) extends Config((site, here, up) => {
  case BuildRoCC => up(BuildRoCC) ++ Seq(
    (p: Parameters) => {
      implicit val q = p
